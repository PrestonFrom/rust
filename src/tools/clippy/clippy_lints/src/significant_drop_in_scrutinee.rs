use crate::significant_drop_in_scrutinee::LintSuggestion::MoveAndClone;
use crate::FxHashSet;
    ...
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>) {
        if let Some(suggestions) = has_significant_drop_in_scrutinee(cx, expr) {
            for found in suggestions {
                ...
                let replacement = found.create_replacement(original, trailing_indent);
                ...
                span_lint_and_then(
                    ...
                    |diag| {
                        // Only output the lint suggestion if it is *not* MoveAndClone. In the
                        // the user would need to clone to avoid the significant drop, just let
                        // them know the issue exists so they can determine the best approach
                        // for their use case.
                        if found.lint_suggestion != MoveAndClone {
                            diag.multipart_suggestion(
                                found.suggestion_message(),
                                vec![
                                    (expr.span.shrink_to_lo(), replacement),
                                    (found.found_span, found.scrutinee_replacement()),
                                ],
                                Applicability::MaybeIncorrect,
                            );
                            if let Some(diag_note) = found.diagnostic_note() {
                                diag.note(diag_note);
                            }
                        }
                    },
...
impl FoundSigDrop {
    fn scrutinee_replacement(&self) -> String {
        if self.is_unit_return_val {
            "()".to_owned()
        } else {
            "value".to_owned()
        }
    ...
    fn create_replacement(&self, original: Cow<'_, str>, trailing_indent: String) -> String {
        match self.lint_suggestion {
            LintSuggestion::MoveOnly => {
                if self.is_unit_return_val {
            ...
            LintSuggestion::MoveAndDerefToCopy => {
                format!("let value = *{};\n{}", original, trailing_indent)
            }
            LintSuggestion::MoveAndClone => {
                format!("let value = {}.clone();\n{}", original, trailing_indent)
            }
        }
    }

    fn suggestion_message(&self) -> &'static str {
        match self.lint_suggestion {
            LintSuggestion::MoveOnly => "try moving the temporary above the match",
            LintSuggestion::MoveAndDerefToCopy => "try moving the temporary above the match and create a copy",
            LintSuggestion::MoveAndClone => "try moving the temporary above the match and cloning the return value",
        }
    }
    
    fn diagnostic_note(&self) -> Option<&'static str> {
        match self.lint_suggestion {
            LintSuggestion::MoveAndClone => Some("this may be expensive for some types"),
            _ => None,
        }
    }
}
...
impl<'a, 'tcx> SigDropHelper<'a, 'tcx> {
    ...
    fn set_current_suggestion_for_binary_op(&mut self, found_span: Span, is_unit_return_val: bool) {
        self.current_sig_drop.replace(FoundSigDrop {
            found_span,
            is_unit_return_val,
            lint_suggestion: LintSuggestion::MoveOnly,
    ...
    fn visit_exprs_for_binary_ops(
        ...
        // If either side had a significant drop, move the entire scrutinee to avoid
        // unnecessary copies and to simplify cases where both sides have significant drops.
        if self.has_significant_drop {
            self.set_current_suggestion_for_binary_op(span, is_unit_return_val);
        }
